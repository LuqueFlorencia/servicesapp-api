...
async function requireAdmin(req, _res, next) {
  try {
    ...
    if (user.role !== "admin")
      return next(new ForbiddenError("No autorizado (se requiere rol admin)."));
...
async function allowSelfOrAdminByDni(req, _res, next) {
  try {
    ...
    if (!isAdmin && !isSelf)
      return next(
        new ForbiddenError("No autorizado (solo el propio usuario o admin).")
      );
