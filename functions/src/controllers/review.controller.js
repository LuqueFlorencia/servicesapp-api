...
async function createReview(req, res) {
  ...
  const data = await reviewService.createReviewData(categoryId, serviceId, payload);
...
async function listReviews(req, res) {
  ...
  return res.status(httpStatusCodes.ok).json(
    getSuccessResponseObject(data, httpStatusCodes.ok, 'OK', 'Reseñas obtenidas.')
  );
}
...
async function getReview(req, res) {
  ...
  return res.status(httpStatusCodes.ok).json(
    getSuccessResponseObject(data, httpStatusCodes.ok, 'OK', 'Reseña obtenida.')
  );
}
...
async function deleteReview(req, res) {
  ...
  const data = await reviewService.deleteReviewId(categoryId, serviceId, reviewId);

  return res.status(httpStatusCodes.ok).json(
    getSuccessResponseObject(data, httpStatusCodes.ok, 'OK', 'Reseña eliminada.')
  );
}
